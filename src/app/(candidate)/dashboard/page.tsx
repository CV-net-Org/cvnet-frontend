...
function SectionCard({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  const { isDark } = useTheme();
  return <div className={`rounded-2xl border ${className} ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>{children}</div>;
}
...
function ConfirmModal({ message, onConfirm, onCancel }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:px-4 bg-slate-900/60 backdrop-blur-sm">
      <div className={`w-full max-w-sm rounded-t-2xl sm:rounded-2xl p-5 sm:p-6 shadow-2xl ${isDark ? 'bg-slate-900 border border-slate-800' : 'bg-white'}`}>
        <div className="flex items-start gap-3 mb-4">
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${isDark ? 'bg-red-900/30' : 'bg-red-50'}`}>
            <AlertTriangle size={16} className={isDark ? 'text-red-400' : 'text-red-600'} />
          </div>
          <div>
            <p className={`text-sm font-bold mb-0.5 ${isDark ? 'text-white' : 'text-slate-900'}`}>Remove role target?</p>
            <p className={`text-xs leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{message}</p>
          </div>
        </div>
        <div className="flex gap-2 pt-1">
          <button type="button" onClick={onCancel}
            className={`flex-1 text-sm font-semibold border py-2.5 rounded-xl transition-colors ${isDark ? 'text-slate-300 border-slate-700 bg-slate-800 hover:bg-slate-700' : 'text-slate-600 border-slate-200 bg-white hover:bg-slate-50'}`}>
            Cancel
          </button>
...
function Skeleton() {
  ...
  return (
    <div className="animate-pulse space-y-5">
      <div className={`rounded-2xl border p-5 h-24 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`} />
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {[1, 2, 3].map(i => (
          <div key={i} className={`rounded-2xl border p-4 flex gap-3 items-center ${i === 3 ? "col-span-2 sm:col-span-1" : ""} ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <div className={`rounded-2xl border h-64 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`} />
        <div className={`rounded-2xl border h-64 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`} />
      </div>
      <div className={`rounded-2xl border h-48 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`} />
    </div>
  );
}
...
export default function DashboardPage() {
    ...
    <header className={`hidden lg:block sticky top-0 z-40 border-b ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span className={`text-sm font-semibold hidden sm:block ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Dashboard</span>
        </div>
  ...
  const DashboardHeader = (
    <div>
      ...
      <p className={`text-sm mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Here's your career progress today.</p>
    </div>
  );
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-5 sm:py-8 space-y-5">
        ...
        <SectionCard className="p-4 sm:p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Active target role</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <select
                ...
                className={`flex-1 min-w-0 px-3.5 py-2.5 text-sm font-semibold border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all cursor-pointer ${isDark ? 'bg-slate-800 border-slate-700 text-blue-400' : 'bg-white border-slate-200 text-blue-700'}`}
              >
              ...
              {activeProfileId && jobRoleProfiles.length > 0 && (
                <button type="button" onClick={() => handleRemoveRole(false)}
                  className={`w-10 h-10 flex items-center justify-center rounded-xl border transition-colors shrink-0 ${isDark ? 'border-slate-700 text-slate-400 hover:text-red-400 hover:border-red-900/50 hover:bg-red-900/20' : 'border-slate-200 text-slate-400 hover:text-red-500 hover:border-red-200 hover:bg-red-50'}`}
            ...
            {!showAddTrack ? (
              <button type="button" onClick={() => setShowAddTrack(true)}
                className={`inline-flex items-center justify-center gap-1.5 text-xs font-semibold border border-dashed px-4 py-2.5 rounded-xl transition-colors sm:w-auto w-full ${isDark ? 'text-slate-400 border-slate-700 hover:border-blue-500 hover:text-blue-400 hover:bg-blue-900/20' : 'text-slate-500 border-slate-300 hover:border-blue-300 hover:text-blue-600 hover:bg-blue-50'}`}>
                <Plus size={13} /> Add new target
              </button>
            ) : (
              <div className={`flex-1 border rounded-xl p-3 space-y-2.5 ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                <div className="flex items-center justify-between">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">New target role</p>
                  <button type="button" onClick={() => setShowAddTrack(false)} className={`hover:text-slate-600 ${isDark ? 'text-slate-500 hover:text-slate-300' : 'text-slate-400 hover:text-slate-600'}`}>
                    <X size={13} />
                  </button>
                </div>
                <div className="flex gap-2">
                  <select value={targetCategory}
                    ...
                    className={`flex-1 px-3 py-2 text-xs font-semibold border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 cursor-pointer ${isDark ? 'bg-slate-900 border-slate-700 text-white' : 'bg-white border-slate-200'}`}>
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          ...
          <SectionCard className="p-5 sm:p-6">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Application breakdown</h2>
                <p className={`text-xs mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Applied, rejected and interviewing</p>
              </div>
            ...
            <div className="flex flex-col gap-6 mt-2">
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                {pieSegments.length === 0 ? (
                  <p className={`text-xs text-center py-4 col-span-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>No applications tracked yet.</p>
                ) : pieSegments.map(({ label, value, color }) => (
                  <div key={label} className={`flex flex-col border rounded-2xl p-4 transition-colors ${isDark ? 'bg-slate-800 border-slate-700 hover:bg-slate-700' : 'bg-slate-50 border-slate-100 hover:bg-slate-100/50'}`}>
                    <div className="flex items-center gap-2 mb-3">
                      <span className="w-2.5 h-2.5 rounded-full shrink-0 shadow-sm" style={{ backgroundColor: color }} />
                      <span className={`text-xs font-bold ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{label}</span>
                    </div>
                    <div className="flex items-end justify-between mt-auto">
                      <span className={`text-2xl font-black tabular-nums leading-none ${isDark ? 'text-white' : 'text-slate-900'}`}>{value}</span>
                      <span className={`text-xs font-bold tabular-nums mb-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
          ...
          <SectionCard className="p-5 sm:p-6 flex flex-col">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Skills gap</h2>
                <p className={`text-xs mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Your level vs. job requirements</p>
              </div>
            ...
            {displaySkills.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-2 text-center py-8">
                ...
                <p className={`text-sm font-semibold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>No analysis yet</p>
                <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Select a target role to see your skill gaps.</p>
              </div>
            ) : (
                  ...
                  return (
                    <div key={i} className="space-y-1.5">
                      <div className="flex items-center justify-between gap-2">
                        <div className="min-w-0">
                          <p className={`text-xs font-bold truncate ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{skill.name}</p>
                          <p className={`text-[10px] font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>You: {LEVEL_LABEL[skill.uVal]}</p>
                        </div>
                        <div className="flex items-center gap-1.5 shrink-0">
                          {met && <CheckCircle2 size={12} className="text-green-500" />}
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-lg border"
                            style={{
                              backgroundColor: met ? (isDark ? "#064e3b" : "#f0fdf4") : (isDark ? "#1e3a8a" : "#eff6ff"),
                              color: met ? (isDark ? "#34d399" : "#15803d") : (isDark ? "#60a5fa" : "#1d4ed8"),
                              borderColor: met ? (isDark ? "#065f46" : "#bbf7d0") : (isDark ? "#1e40af" : "#bfdbfe"),
                            }}>
                            {LEVEL_LABEL[skill.eVal]}
                          </span>
                        </div>
                      </div>
                      <div className={`relative h-2 rounded-full overflow-visible ${isDark ? 'bg-slate-800' : 'bg-slate-100'}`}>
                        ...
                        <div className={`absolute top-[-3px] bottom-[-3px] w-0.5 rounded-full z-10 ${isDark ? 'bg-slate-400' : 'bg-slate-700'}`}
            ...
            {displaySkills.some(s => s.uVal > 0 && s.uVal < s.eVal) && (
              <>
                ...
                <div className="pt-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${isDark ? 'bg-blue-900/30' : 'bg-blue-50'}`}>
                    <Briefcase size={15} className={isDark ? 'text-blue-400' : 'text-blue-600'} />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`text-[10px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Based on your top skill gap</p>
                  </div>
                  <Link href="/skill-gap"
                    className={`shrink-0 text-xs font-semibold border px-3.5 py-2 rounded-xl transition-colors whitespace-nowrap ${isDark ? 'text-blue-400 border-blue-900/50 bg-blue-900/20 hover:bg-blue-900/40' : 'text-blue-600 border-blue-100 bg-blue-50 hover:bg-blue-100'}`}>
                    Explore
                  </Link>
                </div>
              </>
        ...
        <SectionCard>
          <div className={`flex items-center justify-between px-5 sm:px-6 py-4 border-b ${isDark ? 'border-slate-800' : 'border-slate-50'}`}>
            <div>
              <h2 className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Recent applications</h2>
              <p className={`text-xs mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Your latest activity for this track</p>
            </div>
            <Link href="/applications"
          ...
          {!data?.activeProfileData?.recentApps?.length ? (
            <div className="py-14 flex flex-col items-center gap-2 text-center px-6">
              ...
              <p className={`text-sm font-semibold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>No applications yet</p>
              <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Start applying to track your history here.</p>
            </div>
          ) : (
            <>
              {/* Desktop table */}
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {(data?.activeProfileData?.recentApps || []).slice(0, 5).map((app, i) => (
                      <tr key={i} className={`border-b last:border-0 transition-colors ${isDark ? 'border-slate-800 hover:bg-slate-800/50' : 'border-slate-50 hover:bg-slate-50/60'}`}>
                        <td className="px-5 sm:px-6 py-3.5">
                          <p className={`text-sm font-semibold ${isDark ? 'text-slate-200' : 'text-slate-900'}`}>{app.role}</p>
                          <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{app.company}</p>
                        </td>
                        <td className="px-5 sm:px-6 py-3.5">
                          <span className={`text-xs font-medium whitespace-nowrap ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{app.date}</span>
                        </td>
                        <td className="px-5 sm:px-6 py-3.5">
              ...
              <div className={`sm:hidden divide-y ${isDark ? 'divide-slate-800' : 'divide-slate-50'}`}>
                {(data?.activeProfileData?.recentApps || []).slice(0, 5).map((app, i) => (
                  <div key={i} className="px-4 py-3.5 flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-semibold truncate ${isDark ? 'text-slate-200' : 'text-slate-900'}`}>{app.role}</p>
                      <p className={`text-xs truncate ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{app.company} · {app.date}</p>
                    </div>
                    <StatusBadge status={app.status} />
                  </div>
                ))}
